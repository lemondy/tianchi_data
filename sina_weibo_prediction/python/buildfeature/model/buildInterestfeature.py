...
trainpath = r'G:\tianchi\weibo\train\fenci-train-0711'
modelpath = r'G:\tianchi\weibo\train\fenci-model-12'
...
def buildInterest():
    ...
    for root,dirnames,filenames in os.walk(modelpath):
        for filename in filenames:
            modelfile = open(modelpath+'\\'+filename,'r')
            outfile = open(interestpath+'\\'+filename,'ab')
            name = filename.split('.')

            for weibo_content in modelfile:
                weibo = weibo_content.split('\t')
                content = jieba.cut(weibo[6],cut_all=True)
